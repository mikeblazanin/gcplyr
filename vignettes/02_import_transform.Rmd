...
```{r}
temp_filenames <- make_example(vignette = 1, example = 1, dir = ".")
```
...
```{r}
single_filename <- make_example(vignette = 1, example = 2, dir = ".")
```

Let's take a look at what the file looks like:

```{r}
print_df(head(read.csv(single_filename, header = FALSE, 
...
```{r}
widedata_filename <- make_example(vignette = 2, example = 3, dir = ".")
```
...
print_df(head(read.csv(widedata_filename, header = FALSE, 
                       colClasses = "character"), 
              c(10, 10)))
```
...
```{r}
imported_widedata <- read_wides(files = widedata_filename, startrow = 5)
```
...
```{r}
imported_widedata <- read_wides(files = widedata_filename,
