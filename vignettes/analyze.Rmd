...
Notice how some of the minimum density values aren't actually the *initial* density? We can fix that by overriding the default minimum density calculation with `first_min` via the `y0` argument of `lag_time`.
