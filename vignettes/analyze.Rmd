...
If you want to calculate the bacterial maximum growth rate (i.e. their minimum doubling time), it's sufficient to use `R`'s built-in `max` function on the per-capita derivatives we calculated in `vignette("process")`. When using `max`, note that we need to specify `na.rm = TRUE` to tell `max` to ignore all `NA` values.
...
If you would like the minimum doubling time, you can do the same steps as above but using `min` on the `doub_time` values we calculated in `vignette("process")`. Alternatively, you can simply calculate the maximum growth rate as above and convert that into the equivalent minimum doubling time with the `doubling_time` function.
...
ex_dat_mrg_sum <- 
  summarize(group_by(ex_dat_mrg, Bacteria_strain, Phage, Well),
            ...
            doub_time = min(doub_time, na.rm = TRUE),
            doub_time2 = doubling_time(y = max_percap))
...
Bacteria frequently exhibit a second, slower, burst of growth after their first period of rapid growth. This is common in bacterial growth curves and is called *diauxic growth*.
...
We can identify the time when bacteria switch from their first period of rapid growth to their second period by finding a minima in the derivative values. Specifically, the first minima will occur at the beginning of the growth curve, when bacteria are just starting to grow, so we want to identify the second minima. Let's look at some of the derivative values to see this.
...
How can we identify that local minima? We can use the `gcplyr` function `find_local_extrema`. `find_local_extrema` only requires a vector of y data in which to find the local extrema (here `deriv`), and can return a vector of the `index`, `x` values, or `y` values of the extrema it finds. Here we're just going to save the `x` value of the second minima.

At the same time, we're also going to save the density where the diauxic shift occurs. First, we'll save the *index* where the diauxic shift occurs to a column titled `diaxuie_idx`. Then, we can get the `Measurements` value at that index. (Note that it wouldn't work to just specify `return = "y"`, because the `y` values in this case are the `deriv` values).

Note that you can tune the sensitivity of `find_local_extrema` to different heights and widths of peaks and valleys using the `window_width`, `window_width_n`, and `window_height` arguments.
...
In the previous section we identified when bacteria shifted into their second period of rapid growth. If you want to find out what the peak per-capita growth rate was during that second burst, we just have to combine the use of `find_local_extrema`, `max`, and subsetting:
