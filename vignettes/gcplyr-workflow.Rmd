...
example_data_and_designs_sum <-
  summarize(max_abs = max(smoothed))
head(example_data_and_designs_sum)
```

If you want additional measures, you simply add them to the summarize call. For instance, we could get the minimum of the per-capita growth rate:

```{r}
example_data_and_designs_sum <-
  summarize(max_abs = max(smoothed),
            min_percap_deriv = min(deriv_percap))
head(example_data_and_designs_sum)
```
