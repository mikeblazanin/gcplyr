...
```{r}
#What does our (noisy) data look like?
plot(
...
at the growth rate of the bacteria (yes, yes, I know we haven't covered calculating derivatives yet, so just take my word that's what we're plotting here).




```{r}
example_data_and_designs$percap <-
  calc_deriv(y = example_data_and_designs$Measurements,
             x = example_data_and_designs$Time,
             percapita = TRUE, blank = 0,
             subset_by = example_data_and_designs$Well)
             
plot(
  example_data_and_designs$Time[example_data_and_designs$Well == "A1"],
  example_data_and_designs$percap[example_data_and_designs$Well == "A1"],
  xlab = "time", ylab = "Growth rate (per-capita derivative)")
```

Oh. That looks really noisy! If we're going to try to find the maximum growth rate from these data, we're going to need to smooth it first. In fact, this is a common occurrence: at low densities, random noise tends to have a much larger effect than at high densities.
...
**Additionally, all four smoothing algorithms require a tuning parameter** that controls how "smoothed" the data are.
...
lines(example_data_and_designs$Time[
  example_data_and_designs$Well == "A1"],
     log10(example_data_and_designs$smoothed[
       example_data_and_designs$Well == "A1"]))
```

What about our derivative plot?
...
plot(example_data_and_designs$Time[
  example_data_and_designs$Well == "A1"],
     log10(example_data_and_designs$Measurements[
       example_data_and_designs$Well == "A1"]),
  xlab = "time", ylab = "Abs")
lines(example_data_and_designs$Time[
  example_data_and_designs$Well == "A1"],
