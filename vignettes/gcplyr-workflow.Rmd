...
```{r}
#Here we have saved our plate reader file name in the variable temp_filename
data_wide <- read_wides(files = temp_filename)

#Now we transform our data to be tidy-shaped
data_tidy <- 
  trans_wide_to_tidy(wides = data_wide, id_cols = c("file", "Time"))

#Now we import our designs, which we'd previously saved in files named
# Bacteria_strain.csv and Phage.csv
...
data_merged$percap <- calc_deriv(y = data_merged$smoothed,
                                 x = data_merged$Time,
                                 percapita = TRUE, blank = 0,
                                 subset_by = data_merged$Well)

#Calculate the maximum density, area-under-the-curve, and 
#  maximum per-capita growth rate
data_sum <- summarize(
  group_by(data_merged, file, Well, Bacteria_strain, Phage),
  maxdens = max(smoothed, na.rm = TRUE),
  auc = auc(y = smoothed, x = as.numeric(Time)),
  maxpercap = max(percap[!is.infinite(percap)], na.rm = TRUE))
...
### Reading multiple wides from one file
In the rare case that you have multiple wide-shaped datasets saved into a single file, `read_wides` can import that as well. Refer to the earlier section **Reading multiple blocks from one file**, since the syntax for such operations is the same for `read_wides` as it is for `read_blocks`.
...
For **saving tidy-shaped designs** that are less human-readable but easier to import and merge, simply use the built-in `write.csv` function.
...
For **saving block-shaped designs** that are more human-readable but harder to import and merge, use the `gcplyr` function `write_blocks`. `write_blocks` has three options for output format:
...
`calc_deriv` can also return the per-capita derivative. Just as before, provide the x and y values, along with a vector of `subset_by` values (as needed), but now set `percapita = TRUE`. Note that in this case, your **y-values need to have been normalized** so that 0 corresponds to a population density of 0 (e.g. by subtracting the measurement value for a blank)
