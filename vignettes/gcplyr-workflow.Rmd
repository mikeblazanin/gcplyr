...
If you've saved all the files to a single folder, you can easily get a vector with all their names using `list.files`:
```{r}
...
```{r}
head(imported_blockdata, c(6, 10))
```
...
```{r}
print_df(head(imported_widedata, c(6, 8)))
```
...
imported_widedata <- read_wides(files = "widedata.csv",
                                ...
                                metadata = list("experiment_name" = c(1, 2),
                                                "start_date" = c(2, 2)))
print_df(head(imported_widedata, c(6, 5)))
```
