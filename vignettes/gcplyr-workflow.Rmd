...
ex_dat_mrg$deriv_percap_raw <- 
  calc_deriv(x = ex_dat_mrg$Time, y = ex_dat_mrg$Measurements,
             subset_by = ex_dat_mrg$Well, percapita = TRUE, blank = 0)
...
plot(ex_dat_mrg$Time[ex_dat_mrg$Well == "A1"],
     ex_dat_mrg$deriv_percap_raw[ex_dat_mrg$Well == "A1"],
  xlab = "time", ylab = "per-capita growth rate (/sec)", ylim = c(-0.001, 0.03))
```
...
plot(ex_dat_mrg$Time[ex_dat_mrg$Well == "A7"],
     ...
     xlab = "time", ylab = "deriv percap hr")
```
...
ex_dat_mrg_sum <-
  summarize(grouped_ex_dat_mrg,
            growth_rate = max(deriv_percap_hr[smoothed > 0.01], na.rm = TRUE))
head(ex_dat_mrg_sum)
```
