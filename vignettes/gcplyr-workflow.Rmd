...
Now we've moved on to the style of analysis where we are trying to analyze our growth curves so that each one can be characterized by a single number. Some osrt of metric that characterizes some part, or the whole, of a curve.

In order to do so, we will employ the wonderfully powerful `dplyr::summarize` function, along with some functions 
