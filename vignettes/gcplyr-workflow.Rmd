...
`smooth_data` has four different smoothing algorithms: moving average, moving median, loess, and gam. Moving average and moving median are simple smoothing algorithms that primarily act to reduce the strength of outliers on the data. loess and gam are both spline-fitting approaches that smooth data. loess uses polynomial-like curves, which in some cases can create additional curvature artifacts not present in the original data. gam uses additive curves that can, in some cases, avoid this drawback of loess.

To use `smooth_data`, pass your data frame, a formula indicating the response variable (absorbance, density, etc) and the explanatory variable (time), and your method of choice.
