...
* gam is also a spline-fitting approach that tends to better avoid the creation of curvature artifacts

**Additionally, all four smoothing algorithms have a tuning parameter** that controls how "smoothed" the data are. For whichever smoothing method you're using, **you should test smoothing with with multiple different tuning parameter values**, then choose the value that smooths the data as little as is necessary to reduce noise.
...
* For `gam`, see `mgcv::gam` for details, where changing from the default tuning would require passing `formula` and `data` to `smooth_data` via the `...` argument, and altered tuning parameters (e.g. `k`, `sp`, `bs`) would be included in `formula`.
...
We can see that gam has smoothed the data but also introduced substantial biases. In this case you'd likely want to explore combining gam with other smoothing methods, or adjusting the tuning parameters.

### Combining multiple smoothing methods

Often, combining multiple smoothing methods can provide improved results. For instance, `moving-median` is particularly good at removing outliers, but not very good at producing continuously smooth data. In contrast, `moving-average`, `loess`, and `gam` work well at producing continuously smooth data, but aren't as good at removing outliers. Here's an example using `moving-median` and `loess`:
