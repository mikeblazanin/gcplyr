...
#Calculate the maximum density, save it to a column named 'maxdens'
#Calculate the area-under-the-curve, save it to a column named 'auc'
#(these are two common metrics of bacterial growth)
data_sum <- summarize(
...
# Data layouts
With that demonstration done, let's dig into some more details of how data can be organized and what `gcplyr` does. Growth curve data and design elements can be organized in one of three different tabular layouts: block-shaped, wide-shaped, and tidy-shaped, described below.
...
```{r}
print_df(read.csv(temp_filenames[10], header = FALSE, colClasses = "character"))
```
...
If we want to read these files into R, we simply provide `import_blockmeasures` with the vector of file names, and save the result to some `R` object (here, `imported_blockdata`).

```{r}
imported_blockdata <- import_blockmeasures(files = temp_filenames,
                                           wellnames_sep = "")
...
However, running `import_blockmeasures` with only the filenames only works if the data in your block-shaped files starts in the first row and column (or has column names in the first row and/or rownames in the first column). If your data starts elsewhere, `import_blockmeasures` needs to know what row/column to start reading on (if your data isn't the last thing in the file, `import_blockmeasures` also needs to know where your data ends). 
...
```{r}
print_df(read.csv(temp_filenames2[10], header = FALSE, colClasses = "character"))
```

In the above example, the column names are in row 4 and the rownames are in column 1. To specify that to `import_blockmeasures`, we simply do:
...
write_blocks(read_blocks(files = temp_filenames2,
                         startrow = 4, startcol = "A",
                         metadata = list("time" = c(2, "C"))),
...
```{r}
print_df(head(read.csv("blocks_single.csv", header = FALSE, colClasses = "character"), 20))
```
...
Now let's add some metadata. Because we're reading from a single file, we need to specify the metadata slightly differently. Instead of the metadata being a single vector for the row,column location, it's going to be a list of two vectors, one with the row numbers, and one with the column numbers.
...
```{r}
print_df(head(read.csv(temp_filename, header = FALSE, colClasses = "character"), 
...
print_df(head(read.csv(temp_filename2, header = FALSE, colClasses = "character"), 
              c(10, 6)))
```
...
For analysis, `gcplyr` focuses on the first approach: directly quantifying attributes of observed dynamics. If you're interested in the strengths and weaknesses of model-fitting approaches, as well as `R` packages that implement model-fitting methods, check out the **[Other growth curve analysis packages]** section.
