...
# Getting started
`gcplyr` is a package that implements a number of functions to make it easier to import, manipulate, and analyze bacterial growth curves data. This vignette serves as the main walkthrough of how to use `gcplyr`'s most common functions.

If you want to use `gcplyr` to analyze some growth curves data, all you need is the data file with the growth curves measures in it saved in a tabular format (.csv, .xls, or .xlsx) to your computer.

Additionally, users will often want to combine their measures data with some information on the experimental design of their growth curves plate(s). For instance, this might include which strains went into which wells. You can save this information into a tabular file as well, or you can just keep it handy to enter it directly through a function later on.

# Data layouts
Growth curves data and design elements can be organized in one of three different tabular layouts: block-shaped, wide-shaped, and tidy-shaped. 

Tidy-shaped data is the best layout for analyses, but most lab equipment outputs block-shaped or wide-shaped data, and most user-created design files will be block-shaped. Thus, `gcplyr` works by reshaping block-shaped into wide-shaped data, and wide-shaped data into tidy-shaped data, then running any analyses.
...
For example, here is a block-shaped data.frame of a 96-well plate (with ... indicating the additional columns in the middle that are not printed). In this example, all the data shown would be from a single timepoint.
...
For example, here is a wide-shaped dataframe of a 96-well plate (again, ... denotes columns not printed). Each row of this dataframe corresponds to a single timepoint.
...
In tidy-shaped data, each unique measurement has its own row, such that the number of rows equals the number of wells times the number of timepoints. In tidy-shaped data, additional columns specify the timepoint, well information, and any other design elements. Tidy-shaped data is the best format for downstream analyses, and users may be familiar with this format from the `tidyverse` R packages, like `ggplot`. For more details, see: Wickham, Hadley. Tidy data. The Journal of Statistical Software, vol. 59, 2014. for more details.
...
```{r}
print(read.csv(temp_filenames[1], header = FALSE))
```

This would correspond to all the reads for a single plate taken at the very first timepoint. For instance, we can see that the wells on the left-hand-side of the plate have a different density than on the right-hand-side.
...
However, this only works if the data in your block-shaped files starts in the first row and column (or has headers in the first row/column). If your data starts elsewhere, `read_blocks` needs to know what row/column to start reading on (if your data isn't the last thing in the file, `read_blocks` also needs to know where your data ends). For instance, in this set of example files, the timepoint of each block is stored in the 2nd row of the file:

```{r}
print(read.csv(temp_filenames2[1], header = FALSE))
```

Thus, we can see the data header is in row 4, and the data begins in row 5. To specify that to `read_blocks`, we simply do:
...
```{r}
head(read.csv(temp_filename, header = FALSE),
...
However, this only works if the data in your wide-shaped files starts in the first row and column (or has headers in the first row/column). If your data starts elsewhere, `read_wides` needs to know what row/column to start reading on (if your data isn't the last thing in the file, `read_wides` also needs to know where your data ends). For instance, in this set of example files, some other information is output by the equipment in the first few rows:

```{r}
#Let's take a peek at what this file looks like
head(read.csv(temp_filename2, header = FALSE), 
     c(10, 4), row.names = FALSE)
```

Thus, we can see the data header is in row 5, and the data begins in row 6. To specify that to `read_wides`, we simply do:
