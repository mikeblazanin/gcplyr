...
Oh. That looks a lot more noisy! In fact, this is a common occurrence: **at low densities, random noise tends to have a much larger effect** than at high densities. If we're going to be analyzing this data for something like maximum growth rate, we'll need our early density values to fairly smooth.
...
* gam is also a spline-fitting approach, but uses additive curves that have less smoothly changing derivatives, but tends to better avoid the creation of curvature artifacts

**Additionally, all four smoothing algorithms have a tuning parameter** that controls how "smoothed" the data are.

* For `moving-average` and `moving-median`, this is the `window_width_n` parameter, which controls how wide the moving windows used to calculate the median and average is. Specifying the `window_width_n` is required, and larger values will be more "smoothed".
* For `loess`, this is primarily determined by the `span` argument, which can be passed to `smooth_data` via the `...` argument. Specifying the `span` is best practice, and larger values will be more "smoothed".
...
For all of these smoothing methods, **you should visualize smoothing results with different values of the tuning parameter**, then choose the value that gives you the least smoothing necessary.
...
```
We can see that moving-average has helped reduce the effects of noise somewhat, but it's still fairly noisy. We could try using a wider `window_width_n`, we could try other smoothing methods, or we could try combining two smoothing methods.
...
We can see that moving-median has really excluded those major outliers, but the smoothed density (line) is still jumping around a lot. **This is common with moving-median**, so often you may need to try other smoothing methods or combining two smoothing methods.
...
We can see that loess has smoothed the data substantially, but those major outliers are still having an effect. You may need to try other smoothing methods or combining two smoothing methods.
...
lines(
  example_data_and_designs$Time[example_data_and_designs$Well == "A1"],
  log10(example_data_and_designs$smoothed2[example_data_and_designs$Well == "A1"]))
```
