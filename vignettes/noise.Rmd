...
ex_dat_mrg <- 
  dplyr::filter(ex_dat_mrg, Well %in% c("A1", "A2", "A3", sample_wells))
```
...
ggplot(data = dplyr::filter(ex_dat_mrg, Well %in% sample_wells),
  ...
  facet_wrap(~Well, scales = "free_y") +
  ggtitle("average then median smoothing") +
  scale_y_log10()
```
...
Here we can see how the combination of smoothing raw data points and smoothing during derivative calculations can lead to much less noisy derivative outputs. However, our per-capita derivatives are still somewhat noisy in some cases (like Well F10). In the next section, we'll discuss the final strategy for dealing with this sort of per-capita derivative specific noise.
