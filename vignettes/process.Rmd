...
To change the units of your derivatives, use the `x_scale` argument. `x_scale` should be set to the ratio of your `x` units to the units you want. For instance, in our example data `x` is the number of *seconds* since the growth curve began. If we want growth rate in *per-hour*, we set `x_scale = 3600`, since there are 3600 seconds in an hour.
...
ggplot(data = dplyr::filter(ex_dat_mrg, Well %in% sample_wells),
       aes(x = Time/3600, y = deriv_percap_hr)) +
  geom_line() +
  facet_wrap(~Well, scales = "free")
```
